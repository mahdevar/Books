...
\let\LineAlignment\empty
\lst@AddToHook{EveryLine}{\LineAlignment}

\catcode`\⧩=\active
\def ⧩{\let\LineAlignment\hfill\begin{lstlisting}[style=response]}

\catcode`\◮=\active
\def ◮{\end{lstlisting}\let\LineAlignment\empty\removepar}

\catcode`\⧨=\active
\def ⧨{\begin{lstlisting}[style=response]}

\catcode`\◭=\active
\def ◭{\end{lstlisting}\removepar}
